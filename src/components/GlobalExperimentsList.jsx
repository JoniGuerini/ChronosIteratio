...
const GlobalExperimentsList = () => {
    const { gameState, claimMissionReward, getXPRequired } = useGame();
    ...
    const activeMissions = useMemo(() => {
        return MISSIONS
            ...
            .slice(0, 5);
    }, [completedMissions, missionStats, iterons, experimentRank]);
    ...
    return (
        <div className="w-full space-y-4">
            {/* Rank Progress Bar */}
            <div className="flex items-center gap-4 bg-card border border-border rounded-xl px-4 py-3 shadow-sm hover:bg-accent/10 transition-all">
                <div className="flex flex-col min-w-[100px]">
                    <span className="text-[9px] font-black uppercase tracking-[0.2em] text-primary/70 leading-none mb-1">Scientist Rank</span>
                ...
                <div className="flex-1 space-y-1.5">
                    <div className="flex justify-between items-end text-[10px] font-mono font-bold tracking-tight">
                        <span className="text-muted-foreground uppercase opacity-50 tracking-widest">Growth Progress</span>
                        <span className="text-primary/90">{experimentXP} / {xpReq} XP</span>
                    </div>
                    <Progress value={xpPercent} className="h-1.5 bg-primary/10" indicatorClassName="bg-primary shadow-[0_0_10px_rgba(var(--primary),0.3)]" />
                </div>
            </div>
